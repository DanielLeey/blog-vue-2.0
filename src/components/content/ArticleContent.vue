    ...
    getArticle (articleId) {
      this.$http({
        ...
        if (data && data.code === 200) {
          ...
          let articleTags = this.article.articleTags.split(',')
          for (let i in articleTags) {
            if (articleTags[i] !== null && articleTags[i] !== '') { this.tags.push(articleTags[i]) }
          }
          // 更新目录、高亮代码
          this.$nextTick(function () {
            ...
            // this.refreshMobileDirectory()
            document.title = this.article.title + ' | 答案的个人博客 '
          })
        }
      })
    },
